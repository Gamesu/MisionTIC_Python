def clasificar_compuesto(opcion,compuestos_tipo):
    ...
    for x in range(0, len(compuestos_tipo)):
        datosCompuesto =  compuestos_tipo[x] # ('compuesto 1', 'CoO', 'Óxido', '4')
        if datosCompuesto[2] == lista_opciones[int(opcion) - 1]:
            ...
            dic_compuestos[formula] = {'Nombre': nombre, 'Cantidad': cantidad}
            # 'H3S+': {'Nombre': 'compuesto 8', 'Cantidad': '17'}
            total_cantidad = total_cantidad + int(cantidad)
            total_registros = total_registros + 1

    lista_compuesto_seleccionado = (lista_opciones[int(opcion) - 1], dic_compuestos,  total_cantidad, total_registros)
